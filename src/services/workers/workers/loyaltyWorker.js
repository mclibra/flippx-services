...
class LoyaltyWorker extends BaseWorker {
    ...
    async processNoWinCashbackJob() {
        ...
        try {
            ...
            this.log(`No-win cashback completed with ${result.entity.results.length} users processed`);
        } catch (error) {
    ...
    async resetMonthlyReferralCaps() {
        ...
        try {
            ...
            this.log(`Reset referral commission caps for ${loyalties.length} users`);
    ...
    async checkVipDailyLogin() {
        ...
        try {
            ...
            if (!vipConfig || !vipConfig.requirements.dailyLoginRequired) {
                this.log('VIP daily login requirement not configured, skipping check');
                return;
            }
            ...
            for (const user of filteredVipUsers) {
                ...
                const metSessionRequirement = user.sessionTracking?.totalSessionTimeToday &&
                    (user.sessionTracking.totalSessionTimeToday >= requiredSessionMinutes * 60);

                if (!loggedInYesterday || !metSessionRequirement) {
                    this.log(`VIP user ${user._id} failed daily login/session requirement`);
                    this.log(`  - Logged in yesterday: ${loggedInYesterday}`);
                    this.log(`  - Met session requirement: ${metSessionRequirement}`);
                }
            }

            this.log(`Checked daily login requirements for ${filteredVipUsers.length} VIP users`);
        } catch (error) {
            this.logError('Error checking VIP daily login requirements:', error);
        }
    }
    ...
    async updateNoWinTracking() {
        ...
        try {
            ...
            const eligibleTiers = Object.keys(tierConfigs).filter(
                tier => tierConfigs[tier].noWinCashbackPercentage > 0 &&
                    tierConfigs[tier].noWinCashbackDays > 0
            );

            if (eligibleTiers.length === 0) {
                this.log('No tiers have no-win cashback configured, skipping update');
                return;
            }
            ...
            this.log(`Updated no-win tracking for ${loyalties.length} users across tiers: ${eligibleTiers.join(', ')}`);
        } catch (error) {
                ...
                try {
                    ...
                    if (updatedLoyalty && updatedLoyalty.currentTier !== oldTier) {
                        ...
                        if (tierRank[updatedLoyalty.currentTier] > tierRank[oldTier]) {
                            upgrades++;
                            this.log(`User ${loyalty.user} upgraded from ${oldTier} to ${updatedLoyalty.currentTier}`);
                        } else {
                            downgrades++;
                            this.log(`User ${loyalty.user} downgraded from ${oldTier} to ${updatedLoyalty.currentTier}`);
                        }
                    } else {
                        unchanged++;
                    }
                    ...
                    errors++;
                    this.logError(`Error evaluating tier for user ${loyalty.user}:`, userError);
                }
            }

            this.log(`Tier evaluation completed. Upgrades: ${upgrades}, Downgrades: ${downgrades}, Unchanged: ${unchanged}, Errors: ${errors}`);
    ...
    async validateTierConfigIntegrity() {
        ...
        try {
            ...
            this.logError('Error validating tier configuration integrity:', error);
        }
    }
}
