...
class FlippXService {
	...
	static async getCollectionPercentage(gameType) {
		try {
			...
			const defaultPercentage =
				DEFAULT_COLLECTION_PERCENTAGES[gameType.toUpperCase()];
			return {
	...
	static async processWinningCollection(
		userId,
		gameType,
		originalWinAmount,
		ticketId
		...
		try {
			...
			const collectionConfig =
				await this.getCollectionPercentage(gameType);
			...
			const collectionAmount = Math.round(
				(originalWinAmount * collectionPercentage) / 100
			);
	...
	static async updateCollectionTransaction(collectionId, transactionId) {
		try {
			await FlippXCollection.findByIdAndUpdate(collectionId, {
