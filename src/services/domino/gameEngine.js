...
export class DominoGameEngine {
	...
	static canPlaceTile(tile, board) {
		if (board.length === 0)
			return { canPlace: true, sides: ['LEFT', 'RIGHT'] };
	...
	static autoPlay(game) {
		...
		if (validMoves.length > 0) {
			...
			const bestMove = validMoves.reduce((best, current) => {
				...
				// Prefer doubles
				const [currentLeft, currentRight] = current.tile
					.split('-')
					.map(Number);
			...
			while (gameState.drawPile.length > 0) {
				...
				const canPlace = this.canPlaceTile(
					drawnTileFromPile,
					gameState.board
				);
	...
	static processPlaceMove(gameState, move, isAutoMove = false) {
		...
		if (move.tile && move.side) {
			...
			// Place tile on board
			gameState.board = this.placeTileOnBoard(
				move.tile,
				move.side,
				gameState.board,
		...
		if (winnerCheck.isComplete) {
			...
			if (gameState.startedAt) {
				gameState.duration = Math.floor(
					(new Date() - new Date(gameState.startedAt)) / 1000
				);
			}
		} else {
			// Move to next player
		...
		if (emptyHandPlayer) {
			return {
				...
				finalScores: this.calculateFinalScores(
					gameState.players,
					emptyHandPlayer.position,
					gameState.winRule || 'STANDARD'
				),
			};
		}
	...
	static getNextPlayer(gameState) {
		...
		let attempts = 0;
		while (
			!gameState.players[nextPlayer].isConnected &&
	...
	static determineWinner(players, winRule, targetPoints = 100) {
		...
		if (emptyHandPlayer) {
			if (winRule === 'POINTS') {
				...
				const winnerScore = roundScores.find(
					s => s.position === roundWinner
				);
