	...
	dominoNamespace.on('connection', socket => {
				...
				if (result.success) {
					...
					try {
						...
						const baseXP = Math.max(
							5,
							Math.floor(room.entryFee / 3)
						); // 1 XP per $3 entry fee, minimum 5 XP
						const cashTypeMultiplier =
							room.cashType === 'REAL' ? 2 : 1; // Real cash gives more XP
						...
						console.error(
							`Error awarding XP for user ${userId}:`,
							xpError
						);
		...
		socket.on('make-move', async data => {
			try {
				...
				if (!result.entity.success) {
					console.log(
						`Error in manual move by ${socket.userName} `,
						result.entity.error
					);
			...
			if (roomId != null) {
				...
				console.log(
					`User ${userName} successfully ${
						result.isRemoved ? 'left' : 'disconnected'
					} room ${roomId}`
				);
			}
		...
		socket.on('reconnect-to-room', async () => {
			try {
				...
				console.log(
					`User ${socket.userName} reconnected to domino room ${roomId}`
				);
			} catch (error) {
...
const joinOrCreateRoomSocket = async (socket, options) => {
	try {
		...
		if (!gameConfig.isActive) {
			return {
				success: false,
				error: 'Domino games are currently disabled',
			};
		}

		// Validate player count against configuration
		if (
			![2, 3, 4].includes(playerCount) ||
			playerCount > gameConfig.maxPlayersPerRoom
		) {
			return {
				success: false,
				error: `Player count must be between 2 and ${gameConfig.maxPlayersPerRoom}`,
			};
		}
		...
		if (!gameConfig.entryFees.includes(entryFee)) {
			return {
				success: false,
				error: `Entry fee must be one of: $${gameConfig.entryFees.join(
					', $'
				)}`,
			};
		}
		...
		if (!['REAL', 'VIRTUAL'].includes(cashType)) {
			return {
				success: false,
				error: 'Cash type must be REAL or VIRTUAL',
			};
		}
		...
		if (!['STANDARD', 'POINTS'].includes(winRule)) {
			return {
				success: false,
				error: 'Win rule must be STANDARD or POINTS',
			};
		}
		...
		const balance =
			cashType === 'REAL' ? wallet.realBalance : wallet.virtualBalance;
