...
class PayoutService {
	...
	static async getPayoutPercentage(tier, gameType = 'BORLETTE') {
		try {
			...
			const defaultPercentage =
				DEFAULT_PAYOUT_PERCENTAGES[gameType]?.[tier];
			if (!defaultPercentage) {
				throw new Error(
					`No payout percentage found for tier ${tier} and game ${gameType}`
				);
			}
			...
			const defaultPercentage =
				DEFAULT_PAYOUT_PERCENTAGES[gameType]?.[tier] || 60;
			return {
				percentage: defaultPercentage,
				isCustom: false,
				configId: null,
	...
	static async getCurrentConfigurations() {
		try {
			...
			Object.keys(DEFAULT_PAYOUT_PERCENTAGES).forEach(gameType => {
				result[gameType] = {};
				Object.keys(DEFAULT_PAYOUT_PERCENTAGES[gameType]).forEach(
					tier => {
	...
	static async setPayoutConfiguration(
		tier,
		gameType,
		percentage,
		adminUserId,
		...
		try {
			...
			if (percentage < 0 || percentage > 200) {
				return {
					success: false,
					error: 'Payout percentage must be between 0% and 200%',
				};
			}
			...
			const config = new PayoutConfig({
				...
				description:
					options.description ||
					`${tier} ${gameType} payout set to ${percentage}%`,
	...
	static async calculateTierAdjustedPayout(
		baseAmount,
		userTier,
		gameType = 'BORLETTE'
	) {
		try {
			const payoutConfig = await this.getPayoutPercentage(
				userTier,
				gameType
			);
			const adjustedAmount = Math.round(
				(baseAmount * payoutConfig.percentage) / 60
			); // 60% is the base (Silver)
