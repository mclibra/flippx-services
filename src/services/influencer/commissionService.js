...
class InfluencerCommissionService {
	...
	static async processInfluencerCommission(
		refereeId,
		gameType,
		playAmount,
		playId
		...
		try {
			...
			if (!influencerContract) {
				return {
					success: false,
					message: 'Referrer is not an active influencer',
				};
			}
			...
			const now = moment();
			if (
				influencerContract.contractEndDate &&
				moment(influencerContract.contractEndDate).isBefore(now)
			) {
				...
				return {
					success: false,
					message: 'Influencer contract has expired',
				};
			}

			// Get commission configuration
			const commissionConfig =
				influencerContract.commissionRates[gameType.toLowerCase()];
			if (!commissionConfig) {
				return {
					success: false,
			...
			const monthlyEarned =
				monthlyCommissions.length > 0
					? monthlyCommissions[0].totalEarned
					: 0;
			...
			switch (gameType.toLowerCase()) {
				...
				case 'roulette': {
					...
					const totalSpins =
						spinHistory.length > 0 ? spinHistory[0].totalSpins : 0;
				...
				case 'dominoes': {
					...
					const previousWagered =
						wagerHistory.length > 0
							? wagerHistory[0].totalWagered
							: 0;
			...
			if (commissionAmount > 0) {
				...
				const remainingCap =
					commissionConfig.monthlyCap - monthlyEarned;
				...
				await LoyaltyTransaction.create({
					...
					description: `Influencer ${gameType} commission from ${
				...
				return {
					success: true,
					message: `Influencer commission of ${commissionAmount.toFixed(
						2
					)} credited`,
					commissionAmount,
					isInfluencer: true,
	...
	static async resetMonthlyInfluencerCaps() {
		try {
			...
			console.log(
				`Resetting influencer commission caps for month: ${monthKey}`
			);
			...
			return {
				success: true,
				message: 'Monthly caps reset process completed',
			};
		} catch (error) {
