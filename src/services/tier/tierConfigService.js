...
class TierConfigService {
	...
	async validateTierRequirements(tierName, userProgress) {
		...
		if (!tierConfig || !tierConfig.requirements) {
			return {
				isValid: false,
				missingRequirements: ['Tier configuration not found'],
			};
		}
		...
		if (
			requirements.depositAmount30Days &&
			userProgress.totalDeposit30Days < requirements.depositAmount30Days
		) {
			missingRequirements.push(
				`Deposit amount 30 days: $${userProgress.totalDeposit30Days} < $${requirements.depositAmount30Days}`
			);
		}

		if (
			requirements.depositAmount60Days &&
			userProgress.totalDeposit60Days < requirements.depositAmount60Days
		) {
			missingRequirements.push(
				`Deposit amount 60 days: $${userProgress.totalDeposit60Days} < $${requirements.depositAmount60Days}`
			);
		}

		if (
			requirements.depositAmount90Days &&
			userProgress.totalDeposit90Days < requirements.depositAmount90Days
		) {
			missingRequirements.push(
				`Deposit amount 90 days: $${userProgress.totalDeposit90Days} < $${requirements.depositAmount90Days}`
			);
		}

		if (
			requirements.daysPlayedPerWeek &&
			userProgress.daysPlayedThisWeek < requirements.daysPlayedPerWeek
		) {
			missingRequirements.push(
				`Days played per week: ${userProgress.daysPlayedThisWeek} < ${requirements.daysPlayedPerWeek}`
			);
		}

		if (
			requirements.weeklySpendAmount &&
			(userProgress.weeklySpending || 0) < requirements.weeklySpendAmount
		) {
			missingRequirements.push(
				`Weekly spend amount: $${userProgress.weeklySpending || 0} < $${
					requirements.weeklySpendAmount
				}`
			);
		}
		...
		if (
			requirements.dailySessionMinutes &&
			(userProgress.dailySessionMinutesToday || 0) <
				requirements.dailySessionMinutes
		) {
			missingRequirements.push(
	...
	async calculateTierProgress(currentTier, userProgress) {
		...
		if (requirements.depositAmount30Days) {
			progress.depositProgress30Days = {
				...
				percentage: Math.min(
					100,
					((userProgress.totalDeposit30Days || 0) /
						requirements.depositAmount30Days) *
						100
				),
			};
		...
		if (requirements.depositAmount60Days) {
			progress.depositProgress60Days = {
				...
				percentage: Math.min(
					100,
					((userProgress.totalDeposit60Days || 0) /
						requirements.depositAmount60Days) *
						100
				),
			};
		...
		if (requirements.depositAmount90Days) {
			progress.depositProgress90Days = {
				...
				percentage: Math.min(
					100,
					((userProgress.totalDeposit90Days || 0) /
						requirements.depositAmount90Days) *
						100
				),
			};
		...
		if (requirements.daysPlayedPerWeek) {
			progress.playProgress = {
				...
				percentage: Math.min(
					100,
					((userProgress.daysPlayedThisWeek || 0) /
						requirements.daysPlayedPerWeek) *
						100
				),
			};
		...
		return tierConfig
			? {
					...
					weeklyCashbackPercentage:
						tierConfig.weeklyCashbackPercentage,
					monthlyCashbackPercentage:
						tierConfig.monthlyCashbackPercentage,
