...
export const createContract = async (body, admin) => {
	try {
		if (!['ADMIN'].includes(admin.role)) {
			throw new Error(
				'You are not authorized to create influencer contracts.'
			);
		}
...
export const updateContract = async ({ userId }, body, admin) => {
	try {
		if (!['ADMIN'].includes(admin.role)) {
			throw new Error(
				'You are not authorized to update influencer contracts.'
			);
		}
...
export const getInfluencerAnalytics = async (query, admin) => {
	try {
		if (!['ADMIN'].includes(admin.role)) {
			throw new Error(
				'You are not authorized to view influencer analytics.'
			);
		}
