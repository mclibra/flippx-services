...
router.put(
	...
	token({ required: true, roles: ['ADMIN'] }),
	async (req, res) =>
		done(res, await updateContract(req.params, req.body, req.user))
);
...
router.post(
	...
	token({ required: true, roles: ['ADMIN'] }),
	async (req, res) =>
		done(res, await deactivateContract(req.params, req.body, req.user))
);
...
router.get(
	...
	token({ required: true, roles: ['ADMIN'] }),
	async (req, res) =>
		done(res, await getInfluencerAnalytics(req.query, req.user))
);
