...
export const listMegamillion = async query => {
	try {
		...
		// Note: startDate/endDate alone should filter by lottery createdAt, not ticket purchasedOn
		...
		const enrichedLotteriesPromises = lotteries.map(async lottery => {
			...
			// When only dates are provided, they filter lotteries by createdAt, not tickets
			if (
				hasTicketFilters &&
				((startDate && startDate !== 'undefined') ||
					(endDate && endDate !== 'undefined'))
