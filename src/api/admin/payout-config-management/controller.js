	...
	try {
		...
		if (!result.success) {
			return {
				...
				entity: {
					success: false,
					error:
						result.error || 'Failed to get payout configurations',
				},
			};
		}
		...
		return {
			...
			entity: {
				success: false,
				error:
					error.message || 'Failed to retrieve payout configurations',
			},
		};
	}
};
...
export const setPayoutConfiguration = async (body, user) => {
	try {
		...
		if (
			!gameType ||
			!['BORLETTE', 'ROULETTE', 'DOMINOES'].includes(gameType)
		) {
			return {
				status: 400,
				entity: {
	...
	try {
		...
		return {
			...
			entity: {
				success: false,
				error:
					error.message || 'Failed to retrieve configuration history',
			},
		};
	}
};
...
export const validateTierPayoutSystem = async (query, user) => {
	try {
		if (!['ADMIN'].includes(user.role)) {
			throw new Error(
				'You are not authorized to perform system validation.'
			);
		}
		...
		for (const combo of requiredCombinations) {
			const exists = activeConfigs.find(
				config =>
					config.tier === combo.tier &&
					config.gameType === combo.gameType
			);
			if (!exists) {
		...
		validationResults.configurationValidation = {
			...
			promotionalConfigs: activeConfigs.filter(c => c.isPromotional)
				.length,
		...
		const ticketsWithoutPayoutConfig = await BorletteTicket.countDocuments({
			$or: [{ payoutConfig: { $exists: false } }, { payoutConfig: null }],
		});
		...
		const overallHealth =
			validationResults.recommendations.length === 0
				? 'HEALTHY'
				: validationResults.recommendations.some(
							r => r.severity === 'HIGH'
					  )
					? 'CRITICAL'
					: 'WARNING';
		...
		return {
			...
			entity: {
				...
				summary: {
					...
					criticalIssues: validationResults.recommendations.filter(
						r => r.severity === 'HIGH'
					).length,
					warningIssues: validationResults.recommendations.filter(
						r => r.severity === 'MEDIUM'
					).length,
				},
			},
	...
	try {
		...
		for (const scenario of testScenarios) {
			...
			const adjustedResult =
				await PayoutService.calculateTierAdjustedPayout(
					basePayout,
					tier,
					gameType
				);
			...
			testResults.summary.totalAdjustedPayout +=
				adjustedResult.adjustedAmount;
		}

		testResults.summary.totalDifference =
			testResults.summary.totalAdjustedPayout -
			testResults.summary.totalBasePayout;
