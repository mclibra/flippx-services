...
export const buildUserSearchQuery = filters => {
	...
	if (lastLoginStartDate || lastLoginEndDate) {
		...
		if (lastLoginStartDate) {
			query['sessionTracking.lastLoginDate'].$gte = new Date(
				lastLoginStartDate
			);
		}
		if (lastLoginEndDate) {
			query['sessionTracking.lastLoginDate'].$lte = new Date(
				lastLoginEndDate
			);
		}
	}
...
export const validateUserCreationData = userData => {
	const errors = [];
	const { firstName, lastName, email, phone, countryCode, dob, password } =
		userData;
...
export const formatUsersForExport = users => {
	return users.map(user => ({
		...
		'Registration Date': user.createdAt
			...
			: '',
		'Last Login': user.sessionTracking?.lastLoginDate
			? new Date(user.sessionTracking.lastLoginDate).toLocaleDateString()
			: 'Never',
		'ID Proof Status': user.idProof?.verificationStatus || 'NOT_UPLOADED',
		'Address Proof Status':
			user.addressProof?.verificationStatus || 'NOT_UPLOADED',
		Role: user.role || 'USER',
	}));
};
	...
	return {
		...
		gamesPerDay:
			daysSinceRegistration > 0
				? (totalGames / daysSinceRegistration).toFixed(2)
				: 0,
