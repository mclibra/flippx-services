	...
	try {
		...
		if (lastLoginStartDate || lastLoginEndDate) {
			...
			if (lastLoginStartDate) {
				filter['sessionTracking.lastLoginDate'].$gte = new Date(
					lastLoginStartDate
				);
			}
			if (lastLoginEndDate) {
				filter['sessionTracking.lastLoginDate'].$lte = new Date(
					lastLoginEndDate
				);
			}
		}
		...
		const enrichedUsers = await Promise.all(
			users.map(async user => {
				...
				const loyalty = await LoyaltyProfile.findOne({
					user: user._id,
				});
		...
		if (loyaltyTier) {
			finalUsers = enrichedUsers.filter(
				user =>
					user.loyalty &&
					user.loyalty.currentTier === loyaltyTier.toUpperCase()
			);
		}
...
export const getUserDetails = async (userId, query = {}) => {
	try {
		...
		const [
			...
			MegaMillionTicket.find(gameFilters)
				.sort({ createdAt: -1 })
				.limit(5),
			RouletteTicket.find(gameFilters).sort({ createdAt: -1 }).limit(5),
			DominoGame.find({ 'players.user': userId, ...dateFilter })
				.sort({ createdAt: -1 })
				.limit(5),
		]);
		...
		const totalBorletteTickets =
			await BorletteTicket.countDocuments(gameFilters);
		const totalMegaMillionTickets =
			await MegaMillionTicket.countDocuments(gameFilters);
		const totalRouletteTickets =
			await RouletteTicket.countDocuments(gameFilters);
		const totalDominoGames = await DominoGame.countDocuments({
			'players.user': userId,
			...dateFilter,
		});
		...
		const borletteAnalytics = await BorletteTicket.aggregate([
			...
			},
			{
				$addFields: {
					totalLoss: {
						$subtract: ['$totalAmountSpent', '$totalWon'],
					},
					netProfit: {
						$subtract: ['$totalWon', '$totalAmountSpent'],
					},
					wonRate: {
						$cond: [
							{ $eq: ['$totalGames', 0] },
		...
		const combineAnalyticsByCashType = () => {
			...
			[
				...borletteAnalytics,
				...megaMillionAnalytics,
				...rouletteAnalytics,
				...dominoAnalytics,
			].forEach(item => {
				if (item._id && result[item._id]) {
					result[item._id].totalAmountSpent +=
						item.totalAmountSpent || 0;
			...
			['REAL', 'VIRTUAL'].forEach(cashType => {
				// Won Rate
				result[cashType].wonRate =
					result[cashType].totalGames > 0
						? (result[cashType].winningGames /
								result[cashType].totalGames) *
							100
						: 0;

				// Net Profit (positive = profit, negative = loss)
				result[cashType].netProfit =
					result[cashType].totalWon -
					result[cashType].totalAmountSpent;

				// Profit Margin (as percentage)
				result[cashType].profitMargin =
					result[cashType].totalAmountSpent > 0
						? (result[cashType].netProfit /
								result[cashType].totalAmountSpent) *
							100
						: 0;
				...
				userDetails: {
					...
					financialSummary: {
						...
						currentBalance: wallet
							? wallet.virtualBalance + wallet.realBalance
							: 0,
					},
					...
					gamingActivity: {
						totalGames:
							totalBorletteTickets +
							totalMegaMillionTickets +
							totalRouletteTickets +
							totalDominoGames,
			...
			if (!tierResult.success) {
				return {
					...
					entity: {
						success: false,
						error:
							tierResult.error || 'Failed to update loyalty tier',
					},
				};
			}
				...
				switch (action) {
					case 'suspend':
						result = await updateUserStatus(
							userId,
							{ status: 'suspended', reason: data.reason },
							adminUser
						);
						break;
					case 'activate':
					...
					case 'verify':
						if (data.documentType) {
							result = await verifyUserDocument(
								userId,
					...
					case 'updateTier':
						if (data.tier) {
							result = await updateUserLoyalty(
								userId,
								{ tier: data.tier, reason: data.reason },
								adminUser
							);
						}
						break;
