	...
	try {
		...
		return {
			...
			entity: {
				...
				error:
					error.message ||
					'Failed to initialize default tier requirements',
			},
		};
	}
};
		...
		const tierData = {
			...
			benefits: {
				...
				weeklyCashbackPercentage:
					benefits.weeklyCashbackPercentage || 0,
				monthlyCashbackPercentage:
					benefits.monthlyCashbackPercentage || 0,
			...
			requirements: {
				...
				requireIDVerification:
					requirements.requireIDVerification || false,
			...
			referralCommissions: {
				...
				dominoes: {
					per100Wagered:
						referralCommissions.dominoes?.per100Wagered || 0,
...
export const updateTierRequirement = async (id, body, adminUser) => {
	try {
		...
		const { benefits, requirements, referralCommissions, downgrades } =
			body;
