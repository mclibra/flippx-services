...
export const update = async ({ id }, body, user) => {
	try {
		...
		if (virtualCashAmount !== undefined)
			plan.virtualCashAmount = virtualCashAmount;
		if (isAvailableForPurchase !== undefined)
			plan.isAvailableForPurchase = isAvailableForPurchase;
	...
	try {
		...
		return {
			...
			entity: {
				success: false,
				error:
					error.errors || error.message || 'Failed to deprecate plan',
			},
		};
	}
};
	...
	try {
		...
		return {
			...
			entity: {
				success: false,
				error:
					error.errors || error.message || 'Failed to get user plans',
			},
		};
	}
};
