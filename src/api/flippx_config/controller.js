...
export const getCurrentConfigurations = async (query, user) => {
	try {
		if (!['ADMIN'].includes(user.role)) {
			throw new Error(
				'You are not authorized to view collection configurations.'
			);
		}
		...
		return {
			...
			entity: {
				...
				error:
					error.message ||
					'Failed to retrieve collection configurations',
			},
		};
	}
};
...
export const setCollectionPercentage = async (body, user) => {
	try {
		if (!['ADMIN'].includes(user.role)) {
			throw new Error(
				'You are not authorized to set collection percentages.'
			);
		}
		...
		if (
			!gameType ||
			!['BORLETTE', 'ROULETTE', 'DOMINOES', 'MEGAMILLION'].includes(
				gameType
			)
		) {
			return {
		...
		const config = await FlippXConfig.create({
			...
			description:
				description ||
				`FlippX collection set to ${percentage}% for ${gameType}`,
			isActive: true,
		});
...
export const updateCollectionConfiguration = async ({ id }, body, user) => {
	try {
		if (!['ADMIN'].includes(user.role)) {
			throw new Error(
				'You are not authorized to update collection configurations.'
			);
		}
...
export const getCollectionAnalytics = async (query, user) => {
	try {
		if (!['ADMIN'].includes(user.role)) {
			throw new Error(
				'You are not authorized to view collection analytics.'
			);
		}
